...
class AddDataNotifier extends StateNotifier<OpenHouse?> {
  ...
  bool isSelected(Category cat) {
    ...
    return state?.openHouseProperty?.category?.id == cat.id;
  }
  ...
  // Update covered area
  void setCoveredArea(int? coveredArea) {
    state = state?.copyWith(
      propertySize: (state?.propertySize ?? PropertySize()).copyWith(
    ...
    return {
      ...
      "size": {
        ...
        if (model.propertySize?.yearBuilt != null)
          "year_built": CustomDateUtils.formatDateOnlyFilter(
            (model.propertySize?.yearBuilt ?? DateTime.now()).toString(),
          ),
      ...
      "property": {
        ...
        "category": (model.openHouseProperty?.category)?.id,
